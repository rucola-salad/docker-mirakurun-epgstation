<template>
    <div v-if="isAvailable === true && enabled === true" class="jikkyo-overlay" aria-hidden="true">
        <div v-for="comment in comments" :key="comment.id" class="jikkyo-comment" v-bind:class="comment.position" v-bind:style="comment.style">
            {{ comment.text }}
        </div>
    </div>
</template>
...
export default class JikkyoOverlay extends Vue {
    ...
    private getJikkyoId(channel: Channel): string | null {
        if (channel.channelType === 'GR') {
            ...
            const remoteControlKeyId = channel.remoteControlKeyId;
            if (remoteControlKeyId === undefined) {
                return null;
            }
            return map[remoteControlKeyId] || null;
        }
