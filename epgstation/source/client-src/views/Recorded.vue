...
export default class Recorded extends Vue {
    ...
    public gotoDetail(recordedId: apid.RecordedId): void {
        Util.move(this.$router, {
            path: `/recorded/detail/${recordedId.toString(10)}`,
        });
    }
    ...
    public onMultipleEncodeComplete(result: { success: number; failed: number }): void {
        ...
        if (result.failed === 0) {
            this.snackbarState.open({
                color: 'success',
                text: `${result.success} 件のエンコードを追加しました。`,
            });
        } else {
            this.snackbarState.open({
                color: 'error',
                text: `${result.success} 件追加、${result.failed} 件失敗しました。`,
            });
        }
    }
    ...
    public onMultipleJikkyoComplete(result: { created: number; exists: number; failed: number }): void {
        ...
        this.recordedState.fetchData(this.createFetchDataOption()).catch(err => {
            console.error(err);
        });
    }
