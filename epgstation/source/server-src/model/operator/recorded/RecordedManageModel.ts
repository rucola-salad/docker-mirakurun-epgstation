...
export default class RecordedManageModel implements IRecordedManageModel {
    ...
    public async videoFileCleanup(): Promise<void> {
        ...
        for (const file of list.files) {
            ...
            // 実況コメント XML は、同じ basename のファイルが
            // DB の録画ファイル索引に登録されている場合は残す。
            // ts / mp4 など特定の拡張子には依存しない。
            if (path.extname(file).toLowerCase() === '.xml') {
                const basePath = file.slice(0, -4);
                const mediaPrefix = `${basePath}.`;

                const hasRecordedFile = Object.keys(fileIndex).some(
                    indexedFile => indexedFile !== file && indexedFile.startsWith(mediaPrefix),
                );

                if (hasRecordedFile === true) {
                    continue;
                }
            }
