...
export default class RecordedItemUtil implements IRecordedItemUtil {
  ...
  constructor(@inject("IVideoUtil") videoUtil: IVideoUtil) {
  ...
  public convertRecordedToRecordedItem(
    ...
    if (typeof recorded.videoFiles !== "undefined") {
      item.videoFiles = recorded.videoFiles.map((v) => {
        const fullPath = this.videoUtil.getFullFilePathFromVideoFile(v);
        let hasJikkyo = false;

        if (fullPath !== null) {
          const ext = path.extname(fullPath);
          const basePath =
            ext.length === 0 ? fullPath : fullPath.slice(0, -ext.length);
          hasJikkyo = existsSync(`${basePath}.xml`);
        }
        ...
        return {
          ...
          size: v.size,
          hasJikkyo,
          cmState: v.cmState as apid.VideoFileCmState,
        };
      });
    }

    if (
      typeof recorded.dropLogFile !== "undefined" &&
